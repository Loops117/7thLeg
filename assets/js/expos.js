...
(function () {
  ...
  async function fetchExpoAutoApprove(supabase) {
    const res = await supabase
      .from("app_settings")
      .select("bool_value")
      .eq("key", "expo_auto_approve")
      .single();
    if (res.error) throw res.error;
    return !!(res.data && res.data.bool_value);
  }

  async function setExpoAutoApprove(supabase, on) {
    const res = await supabase
      .from("app_settings")
      .update({ bool_value: !!on, updated_at: new Date().toISOString() })
      .eq("key", "expo_auto_approve");
    if (res.error) throw res.error;
    return true;
  }
  ...
  function renderTable(box, rows, state, actions) {
    ...
    box.innerHTML = ''
      ...
      +   '<button id="expo-autoapprove-btn" class="btn btn-sm ' + (state.autoApprove ? 'btn-success' : 'btn-outline-secondary') + '">' 
      +     'Auto-Approve: <strong>' + (state.autoApprove ? 'ON' : 'OFF') + '</strong>'
      +   '</button>'
      + '</div>'
      + '<div class="d-flex align-items-center gap-2">'
...
// AUTO-APPROVE toggle (delegated)
if (!box.dataset.autoapproveWired) {
  box.dataset.autoapproveWired = "1";
  box.addEventListener("click", async function (ev) {
    var tbtn = ev.target.closest("#expo-autoapprove-btn");
    if (!tbtn) return;
    if (!actions || typeof actions.onToggleAutoApprove !== "function") {
      console.warn("onToggleAutoApprove not provided");
      return;
    }
    var next = !state.autoApprove;
    tbtn.disabled = true;
    try {
      await actions.onToggleAutoApprove(next);
      state.autoApprove = next;
      state.draw();
    } catch (e) {
      console.error(e);
      alert("Failed to update Auto-Approve. Check console / RLS policy.");
      tbtn.disabled = false;
    }
  });
}
  ...
  async function boot() {
    ...
    try {
      autoApprove = await fetchExpoAutoApprove(supabase);
    } catch (e) {
      console.warn("Failed to load expo_auto_approve setting", e);
    }

    var STATE = { q: "", sortKey: "created", sortDir: "desc", autoApprove: autoApprove, draw: null };
    ...
    STATE.draw = function () {
      ...
      renderTable(box, filtered, STATE, {
        ...
        onToggleAutoApprove: function (on) { return setExpoAutoApprove(supabase, on); }
      });
    };
