...
async function loadEditSpecies(id) {
  editId = id;
...
// ✅ Back button — switches tab + reattaches dashboard listeners
async function cancelEdit() {
  console.log("📌 Back pressed — returning to Inventory tab");

  // Switch back to Inventory tab
  const inventoryTabBtn = document.querySelector('#dashboardTabs button[data-bs-target="#inventory"]');
  if (inventoryTabBtn) {
    const tab = new bootstrap.Tab(inventoryTabBtn);
    tab.show();
  }

  // Hide edit-species but DO NOT wipe it
  const editPane = document.getElementById("edit-species");
  if (editPane) {
    editPane.classList.remove("show", "active");
  }

  // Run inventory.js if possible
  if (typeof window.loadInventory === "function") {
    window.loadInventory();
  }

  // ✅ Reattach tab event listeners (fixes broken tabs after Back)
  if (typeof tabMap !== "undefined") {
    document.querySelectorAll('#dashboardTabs button[data-bs-toggle="pill"]').forEach(tabBtn => {
      tabBtn.addEventListener('shown.bs.tab', async (e) => {
        ...
        if (tabMap[targetId]) {
          console.log(`📌 Reloading content for tab: ${targetId}`);
          await window.loadTabContent(targetId, tabMap[targetId]);
        }
      });
    });
    console.log("✅ Dashboard tab listeners reattached");
  }
}
