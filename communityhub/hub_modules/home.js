// home.js — bulletins with comments (reply/edit/delete) and avatars
(function () {
  console.log("✅ home.js (bulletins + comments actions + avatars)");
  ...
  function ensureMarkup(){
      ...
      <div class="bulletin-dialog" role="dialog" aria-modal="true" aria-labelledby="bulletin-title">
        ...
        <div class="bulletin-body">
          ...
          <div class="d-flex align-items-center gap-2 mt-2 flex-wrap">
            ...
            <div class="text-secondary small">Images will be resized automatically. HTML allowed; scripts/handlers removed.</div>
          </div>
        </div>
    ...
    st.textContent = `
      ...
      .bulletin-modal.open{ display:flex; align-items:flex-start; justify-content:center; padding: var(--bulletin-modal-top, 220px) 24px 24px; }
      .bulletin-backdrop{ position:absolute; inset:0; background:rgba(0,0,0,.4); }
      .bulletin-dialog{ position:relative; background:#fff; width:min(720px,92vw); border-radius:14px; box-shadow:0 .5rem 1rem rgba(0,0,0,.15); overflow:hidden; max-height: calc(100vh - var(--bulletin-modal-top, 220px) - 32px); display:flex; flex-direction:column; }
      @media (max-width: 576px){ .bulletin-modal.open{ padding: 10vh 3vw 3vh; } .bulletin-dialog{ width: 94vw; max-height: calc(100vh - 13vh); } }
  ...
  function wireModal(){
    ...
    const open = () => {
      ...
      document.getElementById("review-target-type").value = "";
      document.getElementById("review-target-id").value = "";
    };
    const close = () => {
  ...
  function wireComposer(supabase){
    ...
    async function addSelectedFile(){
      ...
      if (!res) { alert("Upload failed (permissions or bucket)."); return; }
    ...
    fileInput?.addEventListener("change", addSelectedFile);
  ...
  const FEED = (() => {
    ...
    async function loadNext(supabase){
      ...
      const limit = 8;
      const rows = await fetchBulletins(supabase, limit, offset);
    ...
    async function renderBulletinCard(supabase, b){
      ...
      try {
        ...
        if (user && user.id === b.user_id) {
          ...
          editBtn?.addEventListener('click', () => {
            ...
            if (reviewBox) reviewBox.classList.toggle('d-none', b.type !== 'review');
      ...
      send?.addEventListener("click", async () => {
        ...
        vbtn.textContent = (list.classList.contains("d-none") ? `View comments (${next})` : "Hide comments");
      ...
      async function reloadComments(){
        ...
        list.innerHTML = items.map(c => `
          <div class="border rounded-3 p-2 mb-2" data-comment-id="${c.id}" data-user-id="${c.user_id}">
            <div class="d-flex align-items-center gap-2 mb-1">
              ${avatarHtml(c.avatar_url || null, c.full_name || "User", 24)}
        ...
        // Unhide edit/delete for own comments
        try{
          const { data: { user } } = await supabase.auth.getUser();
      ...
      // Delegated handlers for reply/edit/delete/save/cancel
      if (!list.dataset.bound){
        list.addEventListener("click", async (ev) => {
          ...
          if (role === "reply-comment"){
            ...
            const inp = cbox.querySelector("input");
            if (inp){
              if (!inp.value.trim()) inp.value = `@${name} `;
              else inp.value = inp.value + ` @${name} `;
              inp.focus();
            }
          }

          if (role === "edit-comment"){
            const row = list.querySelector(`[data-comment-id="${cid}"]`);
            row?.querySelector(`[data-role="edit-row"][data-id="${cid}"]`)?.classList.remove("d-none");
          }

          if (role === "cancel-edit"){
            const row = list.querySelector(`[data-role="edit-row"][data-id="${cid}"]`);
          ...
          if (role === "save-comment"){
            ...
            try {
              const { data: { user } } = await supabase.auth.getUser();
              if (!user) { alert("Please sign in."); return; }
              const { error } = await supabase.from("bulletin_comments").update({ message: newMsg }).eq("id", cid).eq("user_id", user.id);
              if (error) { alert(error.message || "Failed to save."); return; }
              const host = list.querySelector(`[data-comment-id="${cid}"] [data-role="comment-text"]`);
              if (host) host.textContent = newMsg;
              row?.classList.add("d-none");
            } catch (e) {
          ...
          if (role === "delete-comment"){
            ...
            try {
              const { data: { user } } = await supabase.auth.getUser();
              if (!user) { alert("Please sign in."); return; }
              const { error } = await supabase.from("bulletin_comments").delete().eq("id", cid).eq("user_id", user.id);
  ...
  /* ---- Comments fetch with names + avatars ---- */
  async function fetchCommentsWithNames(supabase, bulletinId){
    ...
    if (ids.length){
      const { data: profs } = await supabase
        ...
        .in("id", ids);
      if (profs) for (const p of profs) profileMap[p.id] = { full_name: p.full_name || "User", avatar_url: p.avatar_url || null };
    }
    return comments.map(c => ({
      ...c,
      full_name: (profileMap[c.user_id]?.full_name) || "User",
